...
Thanks to its opinionated directory structure, Nuxt can auto-import your `components/` (broken link), `composables/` (broken link) and `utils/` (broken link).
...
::note
In the `server` (broken link) directory, Nuxt auto-imports exported functions and variables from `server/utils/`.
::
...
Nuxt auto-imports functions and composables to perform data fetching (broken link), get access to the `app context` and runtime config (broken link), manage state (broken link) or define components and plugins.
...
- `components/` for Vue components (broken link).
- `composables/` for Vue composables (broken link).
