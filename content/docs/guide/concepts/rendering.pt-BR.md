...
Middlewares (broken link) and pages (broken link) run in the server and on the client during hydration. Plugins (broken link) can be rendered on the server or client or both. Components (broken link) can be forced to run on the client only as well. Composables (broken link) and utilities (broken link) are rendered based on the context of their usage.
...
**SPA Loading Template** (broken link)
::
...
If you deploy your app to static hosting (broken link) with the `nuxt generate` or `nuxt build --prerender` commands, then by default, Nuxt will render every page as a separate static HTML file.
...
When prerendering a client-rendered app, Nuxt will generate `index.html`, `200.html` and `404.html` files by default. However, if you need to prevent any (or all) of these files from being generated in your build, you can use the `'prerender:generate'` hook from Nitro (broken link).
...
Edge-side rendering is possible thanks to [Nitro](https://nitro.build/), the server engine (broken link) that powers Nuxt. It offers cross-platform support for Node.js, Deno, Cloudflare Workers, and more.
