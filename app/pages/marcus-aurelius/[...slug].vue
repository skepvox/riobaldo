...
const [{ data: page, status }, { data: surround }] = await Promise.all([
  useAsyncData(kebabCase(path.value), () => paintResponse().then(() => nuxtApp.static[kebabCase(path.value)] ?? queryCollection('marcusAurelius').path(path.value).first()), {
    watch: [path]
  }),
  useAsyncData(`${kebabCase(path.value)}-surround`, () => paintResponse().then(() => nuxtApp.static[`${kebabCase(path.value)}-surround`] ?? queryCollectionItemSurroundings('marcusAurelius', path.value, {
    fields: ['description']
  })), { watch: [path] })
])
...
const titleTemplate = computed(() => page.value?.titleTemplate || '%s · Marcus Aurelius')
